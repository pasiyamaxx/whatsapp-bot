...
const { Message, GroupManager } = require('../client');
...
const connect = async () => {
 ...
 const handleMessages = async ({ messages }) => {
  ...
  for (const command of commands) {
   ...
   const execute = (Instance, arguments) => command.function(new Instance(conn, msg), ...arguments, msg, conn, messages[0]);
    ...
    const handlers = {
     ...
     delete: () => {
      if (msg.type === 'protocolMessage') {
       const whatsappMsg = new Message(conn, msg);
       whatsappMsg.messageId = msg.message.protocolMessage.key?.id;
       command.function(whatsappMsg, msg, conn, messages[0]);
      }
     },
    };
    handlers[command.on]?.();
